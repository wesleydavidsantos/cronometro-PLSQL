	...
	BEGIN
		...
		EXCEPTION
			...
			WHEN OTHERS THEN
				...
				ADD_ERRO( 'Falha ao realizar a ação de INICIAR do cronômetro.  Sessão: ' || p_SESSAO_ID  || ' - Quantidade Sessões: ' || LISTA_SESSAO.COUNT  || ' - Erro: ' || SQLERRM );
	...
	BEGIN
		...
		EXCEPTION
			...
			WHEN OTHERS THEN
				...
				ADD_ERRO( 'Falha ao realizar a ação de PARAR do cronômetro. Erro: ' || SQLERRM );
	...
	BEGIN
		...
		EXCEPTION
			...
			WHEN OTHERS THEN
				...
				ADD_ERRO( 'Falha ao realizar a ação de RESET do cronômetro. Sessão: ' || p_SESSAO_ID || '. Erro: ' || SQLERRM );
	...
	BEGIN
		...
		EXCEPTION
			...
			WHEN OTHERS THEN
				...
				ADD_ERRO( 'Falha ao realizar a ação de GET_TEMPO do cronômetro. Erro: ' || SQLERRM );
